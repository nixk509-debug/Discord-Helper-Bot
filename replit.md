...
- **`DISCORD_BOT_TOKEN`** — Bot token from Discord Developer Portal (Bot tab → Token)
- **`DISCORD_CLIENT_ID`** — Application ID from Discord Developer Portal (OAuth2 tab)
- **`DISCORD_CLIENT_SECRET`** — Client Secret from Discord Developer Portal (OAuth2 tab)

### For production with custom domain
- **`PUBLIC_BASE_URL`** — Set to `https://archivistplus.com` in production. When omitted, falls back to `REPLIT_DOMAINS` (Replit subdomain) then `http://localhost:5000`.

### Optional
- **`OWNER_IDS`** — Comma-separated Discord user IDs with permanent admin/premium access. e.g. `123456789,987654321`

### Discord Developer Portal Setup
1. Go to https://discord.com/developers/applications → select your app → OAuth2
2. Add all three Redirect URIs:
   ```
   https://archivistplus.com/auth/discord/callback
   https://<your-replit-subdomain>.replit.app/auth/discord/callback
   http://localhost:5000/auth/discord/callback
   ```
3. Required OAuth2 Scopes: `identify`, `email`, `guilds`
4. Bot invite link (generated by `/api/invite-url`): uses permissions integer `8` (Administrator) and scopes `bot+applications.commands`

### Confirmed working URLs (production)
- `https://archivistplus.com/` — Landing page
- `https://archivistplus.com/auth/discord` — Start Discord OAuth
- `https://archivistplus.com/auth/discord/callback` — OAuth callback (must be in Discord portal)
- `https://archivistplus.com/dashboard` — Dashboard (requires login)
- `https://archivistplus.com/health` — Health check (public)
