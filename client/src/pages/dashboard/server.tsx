...
const automodSchema = z.object({
  ...
  bannedWords: z.string(), // We'll handle conversion to string[] on submit
});
...
export default function ServerSettings() {
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center gap-6 mb-8">
        {server.iconUrl ? (
          <img src={server.iconUrl} alt={server.name} className="w-20 h-20 rounded-2xl shadow-lg shadow-primary/20" />
        ) : (
          <div className="w-20 h-20 rounded-2xl bg-secondary flex items-center justify-center text-2xl font-display font-bold shadow-lg">
        ...
        <div>
          ...
          <p className="text-muted-foreground flex items-center gap-2 mt-1">
            <span className="w-2 h-2 rounded-full bg-green-500 box-shadow-[0_0_10px_rgba(34,197,94,0.5)]"></span>
            Connected • {server.memberCount} Members
          </p>
        </div>
      </div>

      <Tabs defaultValue="general" className="w-full">
        <TabsList className="bg-secondary/50 backdrop-blur-md border border-white/5 mb-6 w-full justify-start h-auto p-1 rounded-xl flex-wrap">
          <TabsTrigger value="general" className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground py-2.5 px-6">
            <Settings className="w-4 h-4 mr-2" /> General
          </TabsTrigger>
          <TabsTrigger value="automod" className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground py-2.5 px-6">
            <Shield className="w-4 h-4 mr-2" /> Automoderation
          </TabsTrigger>
          <TabsTrigger value="logging" className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-primary-foreground py-2.5 px-6">
            <Activity className="w-4 h-4 mr-2" /> Logging
          </TabsTrigger>
        ...
        <TabsContent value="general">
          <GeneralSettingsTab serverId={serverId} settings={server.settings} updateSettings={updateSettings} toast={toast} />
        </TabsContent>
        
        <TabsContent value="automod">
          <AutomodSettingsTab serverId={serverId} settings={server.settings} updateSettings={updateSettings} toast={toast} />
        </TabsContent>

        <TabsContent value="logging">
          <LoggingSettingsTab serverId={serverId} settings={server.settings} updateSettings={updateSettings} toast={toast} />
        </TabsContent>
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} className="bg-background max-w-[200px]" />
                  </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g. 123456789012345678" className="bg-background" />
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea {...field} placeholder="Welcome {user} to {server}!" className="bg-background min-h-[100px]" />
                    </FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea {...field} placeholder="{user} has left the server." className="bg-background min-h-[100px]" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <Button type="submit" disabled={updateSettings.isPending} className="bg-primary hover:bg-primary/90 text-primary-foreground box-glow gap-2 mt-4">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormField
              ...
              name="automodEnabled"
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-xl border border-white/5 bg-background/50 p-6">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 transition-opacity duration-300 ${!form.watch("automodEnabled") ? "opacity-50 pointer-events-none" : ""}`}>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-xl border border-white/5 bg-background/30 p-5">
                    <div className="space-y-0.5">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className={!form.watch("automodEnabled") ? "opacity-50 pointer-events-none" : ""}>
                  ...
                  <FormControl>
                    <Textarea {...field} placeholder="badword1, badword2, scam" className="bg-background min-h-[100px]" />
                  </FormControl>
            ...
            <Button type="submit" disabled={updateSettings.isPending} className="bg-primary hover:bg-primary/90 text-primary-foreground box-glow gap-2">
              <Save className="w-4 h-4" /> 
              {updateSettings.isPending ? "Saving..." : "Save Changes"}
            </Button>
          </form>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} placeholder="e.g. 123456789012345678" className="bg-background max-w-md" />
                  </FormControl>
                      ...
                      <FormField
                        ...
                        render={({ field }) => {
                          return (
                            <FormItem
                              key={item.id}
                              className="flex flex-row items-start space-x-3 space-y-0 rounded-lg border border-white/5 p-4 bg-background/30 hover:bg-background/50 transition-colors"
                            >
                              <FormControl>
                                <Checkbox
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-display font-bold text-glow">Custom Commands</h2>
          <p className="text-muted-foreground text-sm">Create automated responses for specific keywords.</p>
        </div>
        
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button className="bg-primary hover:bg-primary/90 text-primary-foreground box-glow gap-2">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <div className="flex items-center">
                        ...
                        <FormControl>
                          <Input {...field} placeholder="ping" className="rounded-l-none bg-background border-white/10 focus-visible:ring-primary" />
                        </FormControl>
                      </div>
                      <FormMessage />
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea {...field} placeholder="Pong!" className="bg-background border-white/10 focus-visible:ring-primary min-h-[100px]" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <DialogFooter className="pt-4">
                  <Button type="submit" disabled={createCommand.isPending} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground box-glow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {commands.length === 0 ? (
          <div className="col-span-full py-12 text-center glass-card rounded-xl border border-dashed border-white/10">
            ...
            <h3 className="text-lg font-medium">No commands yet</h3>
            <p className="text-sm text-muted-foreground">Create your first custom command to get started.</p>
          </div>
        ) : (
          commands.map((cmd: any) => (
            <Card key={cmd.id} className="glass-card hover:border-primary/30 transition-colors group">
              <CardHeader className="pb-3 flex flex-row items-center justify-between">
                <CardTitle className="text-lg font-mono text-primary bg-primary/10 px-2 py-1 rounded">!{cmd.name}</CardTitle>
                <Button 
                  variant="ghost" 
                  size="icon" 
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground truncate">{cmd.response}</p>
              </CardContent>
            </Card>
          ))
        )}
