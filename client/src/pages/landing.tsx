...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden flex flex-col">
      ...
      {/* Hero */}
      <main className="flex-1 flex flex-col items-center justify-center text-center px-4 relative z-10 max-w-5xl mx-auto w-full pt-12 pb-24">
        <motion.div
          initial="hidden"
          animate="visible"
        ...
        >
          ...
          >
            Power up your community with advanced automoderation, extensive custom commands, seamless logging, and a stunning web dashboard.
          </motion.p>
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 w-full border-y border-white/5 py-8 bg-background/30 backdrop-blur-sm">
            {[
              ...
              <div key={i} className="flex flex-col items-center justify-center">
                <span className="text-3xl md:text-4xl font-display font-bold text-foreground mb-1 text-glow">{stat.value}</span>
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-24 text-left">
            {[
              { icon: Shield, title: "Automoderation", desc: "Keep your server safe automatically with advanced anti-spam, anti-link, and keyword filtering." },
              { icon: Terminal, title: "Custom Commands", desc: "Create unlimited custom commands with simple text responses to engage your community." },
              { icon: BarChart, title: "Detailed Logging", desc: "Never miss an action. Track deleted messages, member joins, leaves, and channel updates in real-time." }
            ].map((feature, i) => (
              <div key={i} className="glass-card p-6 rounded-2xl hover:border-primary/50 transition-colors duration-300 group">
                <div className="w-12 h-12 rounded-xl bg-secondary flex items-center justify-center mb-4 group-hover:bg-primary/20 group-hover:text-primary transition-colors duration-300">
                  <feature.icon className="w-6 h-6 text-muted-foreground group-hover:text-primary" />
                </div>
